...
GROQ_API_KEY = "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"

DATA_PATH = "results_clusters_pacmap.csv"
ROOT_DIR  = os.path.dirname(os.path.abspath(__file__))

# Détection automatique du sous-dossier dans assets/
...
if os.path.isdir(ASSETS_FOLDER):
    print(f"Images : assets/{ASSETS_SUBFOLDER}/ ({len(os.listdir(ASSETS_FOLDER))} fichiers)")
else:
    print(f"Images : dossier assets/{ASSETS_SUBFOLDER}/ introuvable")
...
if os.path.exists(DATA_PATH):
    ...
    df["image_path"] = df["image_path"].apply(lambda x: os.path.splitext(str(x))[0] + ".jpg")
    print(f"{len(df)} oeuvres chargees depuis '{DATA_PATH}'")
else:
    print(f"'{DATA_PATH}' introuvable, generation de donnees simulees")
    np.random.seed(42)
    n = 1700
...
def encode_thumb(rel_path, size=80):
    ...
    try:
        ...
        try:
            ...
            # RGBA pour gerer la transparence
            ...
            # PNG obligatoire pour la transparence (le JPEG ne la supporte pas)
            buf = io.BytesIO()
...
print(f"{len(thumbs)}/{len(df)} miniatures encodees")
...
@app.route("/api/data")
def api_data():
    """Retourne tous les points avec couleurs RGB precalculees."""
    points = []
    for _, row in df.iterrows():
        idx = int(row["index"])
        cid = int(row["cluster"])
    ...
    meta = {
        ...
        "assets_prefix": f"/edionnet-24/assets/{ASSETS_SUBFOLDER}",
    }
...
def api_groq():
    ...
    prompt = (
        f"Tu es un expert en histoire de l'art. Voici un tableau abstrait attribue a {artist_cl}.\n\n"
        "Reponds en francais avec exactement ce format (4 lignes) :\n\n"
        "**Artiste :** [Prenom Nom, nationalite, annees de vie]\n"
        "**Titre probable :** [titre le plus vraisemblable ou \"Sans titre\"]\n"
        "**Date approximative :** [periode estimee, ex: \"vers 1955\"]\n"
        "**Analyse :** [2-3 phrases : couleurs, formes, composition, energie]"
    )
    ...
    # fallback texte si la vision echoue
    try:
        r2 = requests.post(
            "https://api.groq.com/openai/v1/chat/completions",
            ...
            json={
                ...
                "max_tokens": 400,
                "messages": [{"role": "user", "content": (
                    f"Tu es un expert en histoire de l'art. Pour une oeuvre abstraite de {artist_cl}, "
                    "reponds en francais :\n\n"
                    "**Artiste :** ...\n**Titre probable :** ...\n**Date approximative :** ...\n**Analyse :** ..."
                )}]
            }
        ...
        return jsonify({"result": txt + "\n\n_Analyse visuelle indisponible._"})
...
app.wsgi_app = DispatcherMiddleware(NotFound(), {"/edionnet-24": app.wsgi_app})

if __name__ == "__main__":
    print(f"{len(df)} oeuvres, {len(ARTISTS)} artistes, {len(CLUSTERS)} classes")
